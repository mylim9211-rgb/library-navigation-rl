...
# 💡 모델: Baseline (Standard DQN)
MODEL_PATH = "library_advanced_baseline.pt"


# -------------------------------------------------------------
# 1. Standard DQN 구조 (train_advanced_baseline.py와 동일)
# -------------------------------------------------------------
...
def load_policy(env):
    if not os.path.exists(MODEL_PATH):
        print(f"🚨 모델 파일({MODEL_PATH})이 없습니다. Baseline 학습을 먼저 하세요!")
        return None
    ...
    try:
        # train_advanced_baseline.py는 state_dict를 직접 저장했음
        policy.load_state_dict(torch.load(MODEL_PATH, map_location=device))
        policy.eval()
        print(f"📦 Baseline Model(Standard DQN) 로드 완료: {MODEL_PATH}")
...
def eval_mode(env, policy, num_episodes=200, random_start=True):
    ...
    print(f"\n==================================")
    print(f"📉 [Baseline Model | {mode_name}] 성능 평가 ({num_episodes}회)")
    print(f"==================================")
    ...
    for k in target_keys:
        ...
        print(
            f"  - Target {k}: {suc}/{tot} ({rate:.1f}%) | Stuck: {tg['stuck_episodes']} | Timeout: {tg['timeout_episodes']}")
