# train_advanced_tuned.py
import random
from collections import deque

import numpy as np
import torch
import torch.nn as nn
import torch.optim as optim

from env_advanced import LibraryShelfEnvAG, DQN, DuelingDQN, device

# 🔧 튜닝용 하이퍼파라미터 (기존보다 살짝 강화)
EPISODES = 6000          # 에피소드 조금 더 늘리기
BATCH_SIZE = 64
GAMMA = 0.99
LR = 3e-4                # 5e-4 → 3e-4 (조금 더 안정적으로)
MIN_REPLAY_SIZE = 2000   # 버퍼에 조금 더 쌓인 후 학습 시작

RANDOM_START_MIN = 0.0
RANDOM_START_MAX = 0.8   # 커리큘럼 비율은 동일하게 유지


class ReplayBuffer:
    def __init__(self, capacity=100000):
        self.buf = deque(maxlen=capacity)

    def push(self, s, a, r, ns, d):
        self.buf.append((s, a, r, ns, d))

    def sample(self, batch_size):
        batch = random.sample(self.buf, batch_size)
        s, a, r, ns, d = zip(*batch)
        s = np.array(s, dtype=np.float32)
        ns = np.array(ns, dtype=np.float32)
        return (
            torch.tensor(s, dtype=torch.float32, device=device),
            torch.tensor(a, dtype=torch.long, device=device),
            torch.tensor(r, dtype=torch.float32, device=device),
            torch.tensor(ns, dtype=torch.float32, device=device),
            torch.tensor(d, dtype=torch.float32, device=device),
        )

    def __len__(self):
        return len(self.buf)


def eval_policy(env, policy, n_episodes=50):
    policy.eval()
    successes = 0
    steps_list = []

    for _ in range(n_episodes):
        target_idx = random.randint(0, len(env.target_keys) - 1)
        s = env.reset(target_idx=target_idx, random_start=True)

        done = False
        steps = 0
        info = {}
        while not done and steps < env.max_steps:
            state_t = torch.tensor(s, dtype=torch.float32, device=device).unsqueeze(0)
            with torch.no_grad():
                q = policy(state_t)[0]
            a = int(q.argmax().item())
            s, r, done, info = env.step(a)
            steps += 1

        if info.get("reached_goal", False):
            successes += 1
            steps_list.append(steps)

    success_rate = successes / n_episodes if n_episodes > 0 else 0.0
    avg_steps = float(np.mean(steps_list)) if steps_list else None

    print("\n📊 [평가] 랜덤 타겟 + 랜덤 시작 (A~G)")
    print(f"   성공률: {successes}/{n_episodes} ({success_rate*100:.1f}%)")
    if avg_steps is not None:
        print(f"   성공 에피소드 평균 스텝: {avg_steps:.1f}")
    else:
        print("   성공한 에피소드 없음")

    return success_rate, avg_steps


def train_tuned_agent(
    model_path="library_AG_double_dueling_tuned.pt",
    episodes=EPISODES,
):
    env = LibraryShelfEnvAG()
    state_dim = env.state_dim
    n_actions = 4

    # 🔥 튜닝 모델: Double + Dueling DQN
    use_double = True
    use_dueling = True

    policy = DuelingDQN(state_dim, n_actions).to(device)
    target = DuelingDQN(state_dim, n_actions).to(device)

    target.load_state_dict(policy.state_dict())
    opt = optim.Adam(policy.parameters(), lr=LR)
    buf = ReplayBuffer()

    epsilon = 1.0
    eps_min = 0.05
    eps_decay = 0.9995   # 기존보다 조금 천천히 줄이기

    step_count = 0
    rewards_log = []
    success_log = []

    print("🤖 [튜닝 실험] Double + Dueling DQN (Advanced Grid) 학습 시작...")

    for ep in range(episodes):
        frac = ep / episodes
        random_start_prob = RANDOM_START_MIN + (RANDOM_START_MAX - RANDOM_START_MIN) * frac
        random_start_prob = max(0.0, min(1.0, random_start_prob))
        use_random_start = (random.random() < random_start_prob)

        s = env.reset(random_start=use_random_start)

        done = False
        ep_reward = 0.0
        success = False

        while not done:
            if random.random() < epsilon:
                a = random.randint(0, n_actions - 1)
            else:
                state_t = torch.tensor(s, dtype=torch.float32, device=device).unsqueeze(0)
                with torch.no_grad():
                    q = policy(state_t)[0]
                a = int(q.argmax().item())

            ns, r, done, info = env.step(a)
            ep_reward += r
            buf.push(s, a, r, ns, float(done))
            s = ns

            if info.get("reached_goal", False):
                success = True

            if len(buf) >= max(BATCH_SIZE, MIN_REPLAY_SIZE):
                bs, ba, br, bns, bd = buf.sample(BATCH_SIZE)
                q_values = policy(bs).gather(1, ba.unsqueeze(1)).squeeze(1)

                with torch.no_grad():
                    # Double DQN 타깃
                    next_q_online = policy(bns)
                    next_actions = next_q_online.argmax(dim=1)
                    next_q_target = target(bns).gather(
                        1, next_actions.unsqueeze(1)
                    ).squeeze(1)
                    target_q = br + GAMMA * next_q_target * (1 - bd)

                loss = nn.SmoothL1Loss()(q_values, target_q)
                opt.zero_grad()
                loss.backward()
                nn.utils.clip_grad_norm_(policy.parameters(), 1.0)
                opt.step()

            step_count += 1
            if step_count % 1000 == 0:
                target.load_state_dict(policy.state_dict())

        epsilon = max(eps_min, epsilon * eps_decay)
        rewards_log.append(ep_reward)
        success_log.append(1 if success else 0)

        if (ep + 1) % 200 == 0:
            avg_r = float(np.mean(rewards_log[-200:]))
            succ_rate = float(np.mean(success_log[-200:]) * 100)
            print(
                f"[Tuned | ep={ep+1}/{episodes}] "
                f"AvgR(최근200)={avg_r:.3f} | "
                f"Succ={succ_rate:.1f}% | "
                f"eps={epsilon:.2f} | "
                f"rand_start_prob={random_start_prob:.2f}"
            )

    torch.save(
        {
            "state_dict": policy.state_dict(),
            "use_double": use_double,
            "use_dueling": use_dueling,
            "episodes": episodes,
        },
        model_path,
    )
    print(f"\n💾 튜닝 모델 저장 완료: {model_path}")

    # 학습 끝난 뒤 간단히 평가 한 번
    eval_policy(env, policy, n_episodes=50)


if __name__ == "__main__":
    random.seed(42)
    np.random.seed(42)
    torch.manual_seed(42)
    if device.type == "cuda":
        torch.cuda.manual_seed_all(42)

    train_tuned_agent()
